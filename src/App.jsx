...
function ProjectTabsView({ cats, updateCats, user, canEdit, pid, project, state, setState, lang, onDelFolder, standardCatIds }) {
  ...
  const handleChecklistExportDocx = async (selectedIds) => {
    ...
    // v4.7.9: column widths are ABSOLUTE (twips), not percentages.
    // Why: docx renders `{ size: 40, type: WidthType.PERCENTAGE }` as `w:w="40%"`, but OOXML
    // requires w:w to be an integer in FIFTIETHS of a percent when w:type="pct" (5000 = 100%).
    // Word and Google Docs silently discard the malformed value, fall back to autofit, and
    // collapse every table to its minimum content width — that's the "everything crammed onto
    // one side" bug that survived both v4.7.4 and v4.7.5's reformatting attempts.
    // Letter portrait = 12240 twips wide; 0.5" (720 twip) margins each side → 10800 usable.
    const PAGE_W = 12240, PAGE_H = 15840, PAGE_MARGIN = 720;
    const CONTENT_W = PAGE_W - PAGE_MARGIN * 2; // 10800 twips
    const CELL_PAD_X = 80; // per side; subtract 2× from a column to get usable text width
    // v4.7.9: absolute widths sized to the content that actually goes in each column, rather
    // than round percentages. At 10pt Calibri a digit is ~100 twips and "2026-08-21" needs
    // ~1010, so the two date columns must clear ~1170 including padding — at the old 10%
    // (1080) every date wrapped, and "Completed" broke into "Complete / d". Likewise the old
    // 4% "#" column left 192 twips of text space, so any 2-digit row number split in half.
    //   #: 3-4 digits · Task: ~2.6in of wrap space · Status: "Complete" bold
    //   Owner · Target/Completed: one ISO date on one line · Notes: remainder
    const COL_W = [560, 3760, 1180, 1500, 1220, 1220, 1360]; // sums to CONTENT_W (10800)
    ...
    const mkCell = (text, opts = {}) => new DocxTableCell({
      // v4.7.9: DXA (twentieths of a point) — the unit Word itself writes. See COL_W above.
      width: opts.width != null ? { size: opts.width, type: DocxWidthType.DXA } : undefined,
      shading: opts.bg ? { fill: opts.bg } : undefined,
      columnSpan: opts.span,
      margins: { top: 100, bottom: 100, left: CELL_PAD_X, right: CELL_PAD_X }, // twentieths of a point
      borders: cellBorders,
    ...
    const mkHeaderRow = () => new DocxTableRow({
      ...
      children: HEADERS.map((h, i) => mkCell(h, {
        bold: true, color: HEADER_FG, bg: HEADER_BG, width: COL_W[i], center: i === 0 || i === 2, size: 20,
      })),
    });
      ...
      for (const ms of (cat.milestones || [])) {
        ...
        items.forEach((ck, i) => {
          ...
          rows.push(new DocxTableRow({
            children: [
              mkCell(String(itemNum++), { width: COL_W[0], center: true, bg: zebra, color: MUTED }),
              mkCell(ck.label || "", { width: COL_W[1], bg: zebra }),
              mkCell("", {
                width: COL_W[2], center: true, bg: zebra,
                runs: [new DocxTextRun({ text: status, size: 20, font: "Calibri", bold: true, color: statusColor })],
              }),
              mkCell(ck.ownership || "—", { width: COL_W[3], bg: zebra, color: ck.ownership ? undefined : MUTED }),
              mkCell(ck.projectedDate || "—", { width: COL_W[4], bg: zebra, color: ck.projectedDate ? undefined : MUTED }),
              mkCell(
                ck.checkedAt ? String(ck.checkedAt).slice(0, 10) : (ck.actualDate || "—"),
                { width: COL_W[5], bg: zebra, color: (ck.checkedAt || ck.actualDate) ? undefined : MUTED }
              ),
              mkCell(ck.notes || "", { width: COL_W[6], bg: zebra }),
            ],
          }));
        });
        children.push(new DocxTable({
          // v4.7.9: fixed layout + explicit column grid + absolute width. Without FIXED, Word
          // autofits and recomputes every column from its content, discarding the widths above.
          width: { size: CONTENT_W, type: DocxWidthType.DXA },
          columnWidths: COL_W,
    ...
    const doc = new DocxDocument({ sections: [{
      properties: { page: {
        // v4.7.9: pin Letter explicitly — docx defaults to A4 (11906 twips), which made the
        // column math wrong for a US team and left a stray right margin.
